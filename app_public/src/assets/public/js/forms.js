...
function prikaziObvestilo(opozornilnoBesediloStrong, opozorilnoBesedilo = "",tipOpozorila){
    ...
    setTimeout(function(){    document.getElementsByClassName("alert-"+tipOpozorila)[0].style.display = "none";},5000);}
   ...
   if(preveriVrednosti(tabelaCredentialov)){
    ...
    switch(tipForme)
    {
        ...
        if(!new RegExp(regexIzrazi[1]).test(email)){
            prikaziObvestilo("Vnešen napačnen format emaila!"," Uporabi svoj študentski email, ki si ga prejel od UL!","danger");
            event.preventDefault(); return;
         }    
        ...
        case "Registriraj se":
            ...
            if(!new RegExp(regexIzrazi[1]).test(document.getElementById(tabelaCredentialov[1]).value))
            {
        ...
        case "Ponastavi!":
            ...
            if(!(new RegExp(regexIzrazi[0]).test(prvoGeslo)) || prvoGeslo != drugoGeslo){
                prikaziObvestilo("Geslo ni vredu!"," Geslo mora ustrezati naslednjim kriterijem: Geslo more vsebovati vsaj eno veliko in eno majhno črko. Geslo mora vsebovati vsaj eno številko in biti dolgo 8 znakov, hkrati pa naj vsebuje specialni znak glej regex (^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@€#\$%\^&\*])(?=.{8,})). Prav tako poskrbi, da bo v obe polji vpisano isto geslo.","danger");   
                event.preventDefault(); return;
            }
